...
- **CAN-FD is read, on both kinds of CAN log — and the signal decoder can now see past byte
  ...
  `<id>##<flags><data>`, one character different from a classic line, and every one of those lines
  was counted as a line that did not parse — so an FD recording ingested as a log whose every line
  was garbage, or was refused outright. A BLF's `CanFdMessage` and `CanFdMessage64` objects were
  disclosed as undecoded. All three are decoded now.
