...
it is as a velocity command. The one-scalar readings are held to the same rule.
...
own valid-byte count; trusting the code keeps 13 bytes of a 48-byte frame and every signal past
them simply produces no sample. And a `CanFdMessage64` keeps its data **after** its header rather
than inline, so reading it at the inline offset yields forty bytes of header fields decoded as
