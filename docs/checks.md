...
| `autonomy.sequence-complete` | `AUTONOMY.SEQUENCE_COMPLETE` | warning | A rig sensor dropped an aggregate fraction of its frames (default > 5%): its observed frame count falls short of what its own median inter-frame cadence implies over its active span. Catches many small drops that `TEMPORAL.GAP` (single large gap) and `TEMPORAL.RATE` (needs a declared rate MCAP lacks) miss. Robust median baseline; skips streams with too few frames for a stable estimate. |
| `autonomy.ego-pose-continuity` | `AUTONOMY.EGO_POSE_CONTINUITY` | error | The ego trajectory (`Episode.ego_poses`, decoded from Odometry) has a step whose implied speed (distance / elapsed time) exceeds the plausible maximum (default 100 m/s ≈ 360 km/h) — a GPS glitch, localization reset, or stitched log that teleports the ego frame, so every later observation registers against a wrong pose. Reports the worst jump and how many occurred. |
