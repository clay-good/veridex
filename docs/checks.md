...
| `statistical.value-measurability` | the adapter never read values (any MCAP or rosbag2 topic other than a `JointState`, and any `bytes_list` leaf of an RLDS record), or read them but had no stored statistics to compare against (HDF5, Zarr, CAN+DBC, MF4, RLDS numeric leaves, bag `JointState` topics) |
