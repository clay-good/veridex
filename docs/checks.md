...
| `structural.near-duplicate-episode` | `STRUCTURAL.NEAR_DUPLICATE_UNCHECKED` | info | The check abstained, and says on how much: either more episode pairs share frames than it will hold at once (200,000), or every frame of some episodes is held by more than 512 others and was treated as boilerplate, leaving those episodes uncompared. Abstention is reported rather than left silent, because a check that said nothing looks exactly like a check that found nothing. |
...
`veridex check --metadata-only <dataset>` (LeRobot, ROS 2 rosbag2, RLDS/TFDS, and Zarr) answers a narrower question — *does
