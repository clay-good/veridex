...
| `video.media-conformance` | `VIDEO.FPS_MISMATCH` | warning | The container's frame rate (an MP4's sample count over its media duration; a Matroska's declared `DefaultDuration`) differs from the declared rate beyond `rate_deviation` — the same relative tolerance `temporal.rate-conformance` uses. Video time drifts against the action timeline, worsening through the episode. Charged once per stream. |
