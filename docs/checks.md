...
| `autonomy.sequence-complete` | `AUTONOMY.SEQUENCE_COMPLETE` | warning | A rig sensor dropped an aggregate fraction of its frames (> 5%, not yet configurable): its observed frame count falls short of what its own median inter-frame cadence implies over its active span. Catches many small drops that `TEMPORAL.GAP` (single large gap) and `TEMPORAL.RATE` (needs a declared rate MCAP lacks) miss. Robust median baseline; skips streams with too few frames for a stable estimate. |
