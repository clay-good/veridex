...
| `autonomy.point-cloud-density` | `AUTONOMY.POINT_CLOUD_UNDECODED` | error | Some of a point-cloud stream's messages **could not be read as point clouds at all** — the bodies are present and their own length invariants do not hold, which is what a truncated write, a dropped chunk, or a publisher whose point layout disagrees with its own stride leaves behind. The two rules above believe a count only when those invariants hold, so an unreadable body used to be dropped without trace and they then reported a verdict on whichever bodies survived: a LiDAR whose sweeps were four fifths truncated graded exactly as clean as one that was whole, because the surviving fifth were full clouds. The messages still carry a timestamp, a schema and a coordinate frame, so every timing, structural and frame-resolution result counts them as sound frames. A **fault, not an abstention** — the bytes were reached and they are not a cloud, the same fact as `VIDEO.MEDIA_UNREADABLE` one format down. The finding names how many bodies failed, out of how many the stream carried, because the ratio is what a reader acts on. Reported once per stream per episode, independently of the two rules above: a stream can be both mostly unreadable and empty in what did read, and both facts are reported. |
