...
| `structural.content-measurability` | `STRUCTURAL.UNFINGERPRINTED_CONTENT` | info | A stream's frames carry no content fingerprint, so the two checks that prove things by comparing frame bytes — `structural.duplicate-episode` and `structural.stuck-stream` — could not inspect it. The abstention is not a corner case: a LeRobot video feature's pixels live in `.mp4` files outside the Parquet. `stuck-stream` only looks at `Video` streams, which on LeRobot are exactly the hashless ones, so the frozen-camera check never ran there at all. The finding states separately what this cost the duplicate check — that it compared every stream, that it ran on the fingerprinted streams and named the rest (so a duplicate is claimed on partial evidence and a clean result clears only what was compared), or that no episode carries a single fingerprinted stream and it could not run at all. Until 2026-09-11 the middle case did not exist: the duplicate check aborted the whole episode signature if *any* frame of *any* stream lacked a hash, so one video feature — the ordinary layout of a real LeRobot dataset — made two byte-identical episodes undetectable across the entire corpus. |
...
| `structural.duplicate-episode` | `STRUCTURAL.DUPLICATE_EPISODE` | warning | Two or more episodes have identical frame **content** (same schema, timestamps, and per-frame `content_hash`) — a re-upload or a bad merge that over-weights the repeated trajectories. Sound-only: a stream is compared solely when every one of its frames carries a `content_hash`, so it never mis-flags two different same-length episodes that merely share a time base and dataset-global stats. A stream without one is **set aside, not the episode** — requiring every stream to be hashed meant one video feature disabled the check across a whole LeRobot corpus, which is the ordinary shape of a real one. The set-aside names are folded into the signature, so two episodes group together only when the *same* streams were left out of both, and the finding names them: a claim made over part of an episode says so. An episode with no comparable stream at all is still excluded — there is nothing there to prove anything with. A *partial* copy is `structural.near-duplicate-episode`'s. |
...
| Check id | Finding code | Severity | Fires when |
|---|---|---|---|
| `statistical.value-measurability` | `STATISTICAL.UNMEASURED_VALUES` | info | A stream carries neither stored nor recomputed statistics, so every check in this family had nothing to measure on it. Reported for the same reason `TEMPORAL.UNMEASURED_CLOCK` is: MCAP and both generations of ROS bag fingerprint payload bytes without interpreting them — every topic but a `JointState`, an `Imu`, an `Odometry`'s velocity, or one of the messages that are nothing but their own reading — `Twist`/`Wrench` (and their stamped forms), `MagneticField`, `Temperature`, `FluidPressure`, `RelativeHumidity`, `Illuminance`, `Range` — as does any leaf a TFRecord stores as a `bytes_list` (an image, an instruction string), so a recording whose actuator is pinned at its rail reported `data 100` with no statistical findings — while the certificate listed all five statistical checks as run with no categories skipped. (CAN+DBC was the original example, and neither it nor MF4 abstains any more: a DBC decodes each frame into named signal *values*, an MF4 applies its `##CC` conversion to each sample, and both results are measured like any other values.) A saturated actuator, a buried NaN, and a stale stored statistic all signed as checked-and-clean. Informational: a dataset is not worse for the container it was published in; what changes is what a passing verdict is evidence of. **Withheld under `--metadata-only`**, where no adapter reads values by request: there the finding would describe the flag rather than the dataset, and `COVERAGE.METADATA_ONLY` already states that no value was examined. **Calibration channels are excluded**: a `CameraInfo` or a transform tree announces the rig's geometry and has no values to summarize by construction, so listing one among the streams that might hold "a saturated actuator, a NaN, a stuck sensor" is a category error that buries the streams which genuinely went unread. Their content is decoded into the CDM's calibration and graded by the `autonomy.calibration-*` rules, which is where an unusable `CameraInfo` is caught. |
...
| `semantic.task-quality` | `SEMANTIC.NO_TASKS` | info | Episodes carry **no task string at all**, so the task-quality rules had nothing to judge on them. An absent task means the source resolved none — a format with no task concept (MCAP, MF4, CAN, a bag), or a LeRobot dataset without `meta/tasks.jsonl` — which is deliberately not a *fault*: flagging it would fire on every such episode and carry no signal. But not flagging it and not mentioning it are different things, and until this existed the semantic family was the last one whose silence could mean either "judged and sound" or "never asked". A dataset with no instructions at all produced byte-for-byte what a dataset of well-written ones produces. Reported once for the dataset, naming how many episodes and whether *none* of them carries one. **Withheld under `--metadata-only`**: the task is resolved from the data on the formats that have one (a LeRobot episode's from its `task_index` column, an RLDS episode's from a per-step language feature), so a run that opened no payload resolves none in any dataset — the finding would then blame the data for a silence the request caused, and `COVERAGE.METADATA_ONLY` already states the run's shape. Informational, not a defect: a recording is not worse for the format it was published in. What it changes is what a clean semantic result is evidence of. |
...
| `semantic.stream-key-clarity` | `SEMANTIC.AMBIGUOUS_STREAM_KEY` | warning | Two *distinct* stream keys differ only by case or whitespace. A naming mistake is a property of the dataset's schema, so each collision is reported once, naming the first episode it appears in — not once per episode. At most **four** of the other members are named, followed by a count of the rest: the group is reported once per member, and nothing caps how many streams a file may declare, so listing every other member made the report quadratic in the stream count — 2,000 streams that normalize alike produced 263 KB per message and roughly half a gigabyte of report, in the terminal, the JSON, the SARIF and the HTML alike. |
...
| `semantic.annotation-integrity` | `SEMANTIC.ANNOTATION_CONFLICT` | warning | Two `language` annotations at the same timestamp carry different values — contradictory supervision for one instant. The full count of conflicting values is always stated; at most **four** of the values themselves are named, followed by a count of the rest, because nothing caps how many annotations a file may stamp on one instant. |
...
| `semantic.annotation-integrity` | `SEMANTIC.NO_ANNOTATIONS` | info | The dataset carries **no `language` annotation at all**, so the alignment, conflict and empty-annotation rules had nothing to judge. The same rule as its sibling above, and the common case rather than the rare one: LeRobot surfaces only mid-episode `task_index` changes as timestamped `language` labels, so a single-task dataset carries none. Three rules that found nothing where there was nothing to find is not the result those rules produce over a dataset full of annotations, and the report said the same thing either way. Reported once for the dataset, and **withheld under `--metadata-only`** for the same reason as its sibling above — an annotation is data. |

## Video — does the media match the data it is paired with?

A video dataset is two artifacts nothing reconciles: a manifest and a data table on one side, a
video container on the other. A loader pairs video frame *i* with data row *i* and asks no
questions. These checks read the container's **headers only** — Veridex never decodes a pixel — so
"decodable" is answered as "does this container parse and describe a video track", which is what
catches the truncated, half-uploaded, and re-encoded files.
...
layout, in any of `.mp4` / `.m4v` / `.mov` / `.mkv` / `.webm` — which container a file *is* comes
from its magic bytes rather than its extension. A container Veridex cannot walk at all is reported
as unreadable and named; a container family it does not collect would instead be *not found*, which
is why the extension list and the magic-byte dispatch are kept in step.
...
| `video.media-readable` | `VIDEO.MEDIA_UNREADABLE` | error | The file exists but is not a readable container — the finding names the structure that was wrong (no `moov` box, no Segment element, a truncated header, an element declaring more bytes than remain). A container that will not parse will not decode, and training fails at that episode hours into a run. |
| `video.media-conformance` | `VIDEO.FRAME_COUNT_MISMATCH` | error | The container's frame count — an MP4's sample table, or the blocks a Matroska's clusters hold, a laced block counting the frames it laces — differs from the frames the paired data stream carries. Every video/data pair past the shorter of the two is wrong, so the policy learns actions against images from a different moment. Reported per episode — one bad video is one bad episode — **except** when every episode of a stream whose video could be *measured* is off by the same amount, and at least two could be, which is one systematic export defect (an encoder dropping a leading frame, a converter counting from one) and is charged once. An episode whose file is missing, whose container would not parse, or whose container declares no sample count was never weighed, so it neither supports nor defeats the pattern — and a stream with only one measured episode is reported per episode, naming the file and both counts. Rolling it up changes how often it is reported, never its severity. |
| `video.media-conformance` | `VIDEO.FRAME_COUNT_UNMEASURED` | info | Every container for a stream was readable and **none of them states how many frames it holds**, so the comparison above never ran on it. A fragmented MP4 keeps its samples in `moof` fragments and leaves the sample table in `moov` empty — what `ffmpeg -movflags frag_keyframe+empty_moov`, DASH/CMAF and most hardware recorders write — and a live-muxed Matroska whose clusters declare no size is the same case. Reported once per stream, and only when *no* episode could be measured: where some could, the mismatch rollup already speaks for those. Without it a video the family never compared read exactly like one it compared and found sound. |
...
| `video.media-conformance` | `VIDEO.CODEC_MISMATCH` | warning | The container's codec differs from the declared one. Both names must resolve through the alias table (`h264`/`libx264`/`h264_videotoolbox`/`avc1`/`V_MPEG4/ISO/AVC`, `hevc`/`hvc1`, `av1`/`av01`/`V_AV1`, `vp9`/`vp09`, `vp8`, `mpeg4`, `mjpeg`, `prores` — a Matroska spells the same encoder as a `CodecID`, and the table resolves both spellings to one) — encoder names are an open namespace, so a spelling Veridex does not recognize means *cannot tell*, and the check abstains rather than calling it a difference. Charged once per stream, per distinct value. |
| `video.media-conformance` | `VIDEO.FPS_MISMATCH` | warning | The container's frame rate (an MP4's sample count over its media duration; a Matroska's declared `DefaultDuration`, or — when it declares none, as a variable-rate file does — the rate its own block timestamps show) differs from the declared rate beyond `rate_deviation` — the same relative tolerance `temporal.rate-conformance` uses. Video time drifts against the action timeline, worsening through the episode. Charged once per stream. |
| `video.media-conformance` | `VIDEO.FRAME_GAP` | warning | The container's own per-frame timing records a gap of **two or more frame intervals** between two consecutive frames — a whole frame period in which nothing was recorded. This is the one video fault an average cannot express: frames per second is frames over elapsed time, so a camera that froze for two seconds in the middle of a minute-long clip still divides out to exactly its declared rate, and every comparison above passes on a recording with a hole in it. Measured from an MP4's `stts` (the largest `sample_delta` over the entries describing at least one sample) or a Matroska's own block timestamps, and compared against the container's *observed* rate rather than the manifest's — so this says the file disagrees with itself, and a file encoded at the wrong rate stays one defect reported once as `VIDEO.FPS_MISMATCH`. The threshold is two intervals and is **not configurable**: a frame period that passed with no frame means the same thing on every dataset, and integer timestamps in units of one frame cannot express a step of 1.5, so it never sits inside the quantization noise. Reported per episode — a stall is one recording's fault, not an export's. |
| `video.media-conformance` | `VIDEO.FRAME_TIMING_UNMEASURED` | info | Every container for a stream was readable and **none of them states per-frame timing this reader can follow**, so the gaps between its frames were never measured. A fragmented MP4 leaves its timing in `moof` fragments as it does its sample count; a Matroska track carrying B-frames stamps its blocks in presentation order while storing them in decode order, which one pass cannot read as elapsed time, and a laced block holds several frames under one timestamp. Reported once per stream, and only when *no* episode could be measured. Without it, a recording with a hole in it reads here exactly like one without — which is the same silence `VIDEO.FRAME_COUNT_UNMEASURED` exists to break, one dimension over. |
...
| `provenance.completeness` | `PROVENANCE.MISSING_CALIBRATION` | info | No calibration is known. Silent on a run that opened no stream payload (`--metadata-only`): a bag's calibration lives in its message bodies, so a run that declined to read them cannot tell an uncalibrated dataset from one it did not look at — the same reason `AUTONOMY.CALIBRATION_INCOMPLETE` abstains there. `PROVENANCE.MISSING_UPSTREAM` is silent there for the same reason (RLDS records lineage inside the TFRecord); every other expected element comes from a manifest, header or dataset card, which such a run does read, so its absence means the same thing in either mode. A recording that carries its own extrinsics and camera intrinsics — a ROS transform tree and `CameraInfo`, from MCAP or from either generation of bag — supplies this element itself (`Known`, "recorded in-band"), because the calibration is *in* the dataset and in its content hash, which answers the question better than a reference to a file the reader cannot check. |
...
| ROS 1 rosbag | `recorder` from the bag header's `callerid`, plus the same in-band `calibration` as MCAP — decoded from the recording's own `TFMessage`s and `CameraInfo`s |
...
| Check id | Finding code | Severity | Fires when |
|---|---|---|---|
| `autonomy.rig-sync` | `AUTONOMY.RIG_SYNC_UNCOMPARED` | info | A rig episode where fewer than **two** sensor streams have a measurable time span, so the cross-sensor drift this check exists to measure was never compared. A rig reaches this with a sensor that published once — a driver that emitted a single scan and stopped. Reported per episode. It matters more than most abstentions: `world-model-ready` judges its "rig sensors within a 20 ms cross-sensor span drift" criterion on this check, and a criterion passes only when its check ran and found nothing — so without this the rig certified ready on a comparison that never happened. Suppressed under a metadata-only run, where no stream has frames by request and `COVERAGE.*` already says so. |
...
| `autonomy.sequence-complete` | `AUTONOMY.SEQUENCE_UNMEASURED` | info | Rig sensors that carry neither publisher numbering nor a cadence regular enough to imply how many frames were due — too few frames to establish one, or intervals so irregular that a gap means an idle bus rather than a loss. Reported once per episode, naming the streams. Only sensor streams count toward it: a rig log also carries `/rosout`, a latched transform tree and a `CameraInfo` channel, none of which has a cadence whose gaps mean dropped observations. `world-model-ready` judges "no rig sensor dropping more than 5% of its frames" on this check, so without this a rig whose sensors were all unjudgeable certified against a measurement none of them received. Suppressed under a metadata-only run. |
| `autonomy.sequence-complete` | `AUTONOMY.SEQUENCE_COMPLETE` | warning | A rig sensor dropped an aggregate fraction of its frames (> 5%, configurable via `sequence_drop_fraction`): its inter-frame gaps sitting at *multiples* of its own median cadence account for the frames those gaps swallowed. Counting multiples rather than dividing the span by the cadence is what keeps an idling event-driven signal from being called incomplete. Catches many small drops that `TEMPORAL.GAP` (single large gap) and `TEMPORAL.RATE` (needs a declared rate MCAP lacks) miss. Robust median baseline; skips streams with too few frames for a stable estimate, and abstains on an event-driven signal whose intervals are far from uniform (no cadence to fall short of — that shape is `TEMPORAL.JITTER`'s). **Only used where the publisher did not number its messages** — where it did, `AUTONOMY.SEQUENCE_DROPPED` counts the loss instead, and an estimate must never be allowed to contradict a count. |
| `autonomy.sequence-complete` | `AUTONOMY.SEQUENCE_DROPPED` | warning | A rig sensor lost more than `sequence_drop_fraction` (default 5%) of its messages, **counted** from the publisher's own per-message `sequence` rather than estimated from timing: a hole in the numbering is direct evidence that a message was published and never reached the file. Needs no cadence, so it answers on the event-driven streams the estimate above abstains on, and no minimum frame count, so it answers on short ones. It also sees the shape the estimate cannot: losses scattered one message at a time leave intervals near twice the cadence, which is exactly what ordinary jitter leaves, so a stream bleeding a tenth of its messages can pass every timing check with a cadence that still looks regular. Read from MCAP, from a ROS 2 rosbag2 recorded with the MCAP storage plugin, and from a **ROS 1 bag**, where the counter is `header.seq` — the one field of the three that ROS 2 dropped, so a `.bag` answers this where the same rig recorded to a `.db3` cannot (that table has no such column, and falls back to the estimate). Silent where the publisher never used the field (every message left at 0) — an unused counter is not a stream whose every message went missing. |
| `autonomy.sequence-complete` | `AUTONOMY.SEQUENCE_RENUMBERED` | warning | A rig sensor's message numbering re-used a number or ran backwards: the publisher restarted mid-recording, or a message was delivered twice. Reported instead of a drop fraction, not alongside one — a hole below a restart is the distance between two unrelated counts, not a loss, so completeness for that stream is unverified rather than clean. |
| `autonomy.ego-pose-continuity` | `AUTONOMY.EGO_POSE_UNMEASURED` | info | An episode carries an ego trajectory that could not be judged: fewer than two poses — localization that published once and stopped — or no step whose timestamp advances, which leaves no distance-over-time to compute. Reported per episode. `world-model-ready` judges its "ego trajectory continuous" criterion on this check, and a criterion passes only when its check ran and found nothing, so without this such a trajectory certified as continuous on a comparison that never happened. A trajectory whose *coordinates* broke is the separate `AUTONOMY.EGO_POSE_NON_FINITE` error, not this. Suppressed under a metadata-only run, where a trajectory carries no poses by request. |
...
| `autonomy.sensor-frame-resolution` | `AUTONOMY.CAMERA_FRAME_UNRELATED` | error | The rig's **cameras sit in more than one disconnected part** of the transform tree, so no sensor can be projected into both. The per-sensor rule above is answered from *any* camera, which is what it should be — a sensor that reaches one camera can be projected into it. On a multi-camera rig that leaves a gap the rule cannot see: two sensor pods with only one wired to the body — a rig whose second pod's extrinsics were never recorded — gives every sensor a camera in its own component, so every per-sensor result passes and `world-model-ready` certifies, while nothing on the rig can be projected into the other pod's camera and every image it recorded is unusable for fusion. Reported once per episode, naming which cameras are stranded from which. Silent on the ordinary multi-camera vehicle, whose cameras share one component. |
| `autonomy.sensor-frame-resolution` | `AUTONOMY.SENSOR_FRAME_UNCHECKED` **Withheld under `--metadata-only`**: the transform tree is built from `TFMessage` bodies and no adapter reads one from a manifest, so under the flag this said "the dataset declares no transform tree" about datasets that declare one — a false statement about the recording rather than a hedge about the run. |
| `semantic.task-quality` | `SEMANTIC.NO_TASKS` |
| `semantic.annotation-integrity` | `SEMANTIC.NO_ANNOTATIONS` | info | The dataset declares no transform tree at all, so no sensor's frame could be resolved through one. The missing tree itself is a defect and is reported once, by `autonomy.calibration-completeness`; this check must not accuse the rig a second time for the same missing document. But returning *nothing* was not the same as saying nothing — a readiness criterion is judged by its own check's findings, so `world-model-ready` signed "✓ every sensor's own frame resolves through the tree to a camera" beside the failing calibration criterion, over a rig with no tree to resolve through. `ready` was already false from that other criterion, so nothing was certified that should not have been, but the sentence was untrue about the run and a reader deciding what to fix was told the spatial wiring was sound. Abstaining out loud is a statement about the check rather than a second accusation about the data. **Raised only for a rig.** This check's faults are per stream and self-limiting — a dataset whose sensors declare no frames produces none — but the abstention fires on the *absence* of a tree, which almost nothing outside an autonomy rig carries, so ungated it told every LeRobot arm recording and every ordinary MCAP file that it declares no transform tree: true, and not a gap in anything they were being measured for. |
...
| `autonomy.gnss-plausibility` | `AUTONOMY.GNSS_IMPLAUSIBLE` | error | A GNSS stream records a latitude outside ±90° or a longitude outside ±180° — not a place. The receiver, the unit conversion, or the field order is wrong, and every downstream use of the trajectory is wrong with it *silently*, because the numbers still look like coordinates. Judged from the per-dimension statistics the `NavSatFix` decode produces, so the flagged value is a real recorded one. Reachable for MCAP, ROS 2 rosbag2 and ROS 1 rosbag, the three sources that carry a `NavSatFix`; silent where the GNSS was never decoded, which `STATISTICAL.UNMEASURED_VALUES` reports instead — a check that cannot see the values must not report them plausible. |
| `autonomy.gnss-plausibility` | `AUTONOMY.GNSS_UNMEASURED` | info | A `Gnss` stream carries no decoded coordinates, so the plausibility rules had nothing to measure on it. The absence was previously left to `STATISTICAL.UNMEASURED_VALUES`, which does report it — but that finding belongs to `statistical.value-measurability`, a *different check id*, and a readiness criterion is judged by its own check's findings. So the silence was disclosed in the report while `world-model-ready` counted this check as passed, attesting "every satellite fix is a possible place" over coordinates nobody decoded. A deferral cannot carry a criterion. Reported once for the dataset, naming the streams; informational, and never an accusation about the coordinates themselves. **Withheld under `--metadata-only`**, for the reason its sibling is: the coordinates come from a body, so their absence there is the request's doing rather than the receiver's. |
...
| `autonomy.gnss-fix-availability` | `AUTONOMY.GNSS_NO_FIX` | error | A satellite receiver stamped more than `gnss_unfixed_fraction` of its messages (default 0.5; `world-model-ready` tightens it to 0.05) with `NavSatStatus.STATUS_NO_FIX` — the receiver stating that the latitude and longitude beside it are not a position. Those coordinates are whatever the driver left behind, so Veridex records no values for them, and that is exactly what makes the outage invisible: the message still produced a frame, on time, so the stream keeps its frame count, its cadence and its span, and every temporal check reads it as continuous. The fixes that did land are plausible, so `autonomy.gnss-plausibility` passes on them. A recording whose receiver had no fix for four fifths of a drive was otherwise identical, in a report, to one that never lost it. Judged as a fraction rather than on any outage at all, because a brief one is a fact about the road — a tunnel, an underpass, an urban canyon — and not a fault in the data. |
| `autonomy.gnss-fix-availability` | `AUTONOMY.GNSS_STATUS_UNREAD` | info | A `Gnss` stream carries no decoded fix status, so the availability rule had nothing to measure on it. This matters more than most abstentions, because the fault the rule exists to catch is invisible to every other check — an outage leaves the frame count, the cadence and the span intact — so a receiver nobody decoded is, in a report, exactly what a healthy one looks like. Not an accusation: the absence of the status is not an absence of fixes, and `AUTONOMY.GNSS_NO_FIX` is never raised from it. Reported once for the dataset, naming the streams. Informational — a dataset is not worse for the container it was published in; what changes is what a passing result is evidence of, and `world-model-ready` judges its fix-availability criterion on this check, so an undecoded receiver now refuses that criterion instead of meeting it. **Withheld under `--metadata-only`**: the status is decoded from a `NavSatFix` body, so a run that opened none read no status on any dataset, and the finding would say a receiver carries none when it carries it and the run declined to look — `COVERAGE.METADATA_ONLY` states that instead. |
| `autonomy.point-cloud-density` | `AUTONOMY.POINT_CLOUD_EMPTY` | error | Every message on a LiDAR, radar or planar-scanner stream carried **zero points** — a `PointCloud2` declaring `height × width` of zero, or a `LaserScan` none of whose returns fell inside the scanner's own `[range_min, range_max]`, which is how REP 117 says a driver reports nothing there. This is the autonomy fault every other check passes: a driver that lost its sensor keeps publishing perfectly-formed clouds at the configured rate, with the right schema, the right `frame_id`, monotonic timestamps and no jitter — so the structural family sees frames, the temporal family sees a clean 10 Hz, `autonomy.sensor-frame-resolution` places the sensor in the tree, and the rig certifies as world-model-ready on a LiDAR that recorded nothing. Read from each message's own `height × width`, which a `PointCloud2` states in its header ahead of the bulk blob — no point payload is decoded. A count is believed only when the body proves it is a `PointCloud2`: the decode walks the field list to the `point_step`/`row_step`/`data` values behind it and requires the message's own invariants to hold (at least one declared field, a non-zero point stride, a record layout that fits that stride — every field's `offset + width × count` inside the point, no two fields sharing a byte, and no datatype tag the message definition does not define — a `row_step` covering a row of `width` points, `data` of exactly `row_step × height` bytes, and those bytes actually present). A channel's declared schema is not proof of its bodies — a mislabelled topic, a truncated write or a stubbed payload would otherwise yield a fabricated count and a finding about honest data. Reported once per stream per episode. Silent where no point counts were read at all (every non-ROS format, and a `--metadata-only` run): a stream whose density was never measured is not a stream measured and found empty. |
...
| `autonomy.image-integrity` | `AUTONOMY.IMAGE_EMPTY` | error | Every `sensor_msgs/msg/Image` message on a camera stream declared **no pixels** — a zero width or a zero height. The point-cloud fault one modality over, on the modality a policy is usually trained on: a driver that lost its sensor keeps publishing perfectly-formed frames at the configured rate, with the right schema, the right `frame_id`, monotonic timestamps and no jitter — so the structural family sees frames, the temporal family sees a clean 30 Hz, `autonomy.sensor-frame-resolution` places the camera in the tree, and the rig certifies as world-model-ready on a camera that recorded nothing. Read from each message's own `height`/`width`, which an `Image` states in its header ahead of the pixel blob — no pixel data is decoded. A `CompressedImage` is graded the same way, from the JPEG or PNG frame header inside its payload (most real bags record their cameras compressed, and the fault has to be caught on both spellings of the topic); a codec with no header parser here is **untried**, so the stream abstains rather than being accused, and a payload of zero bytes is recognized as empty without any codec at all. A size is believed only when the body proves it is an `Image`: the decode requires a non-empty `encoding` (the field an all-zero body cannot satisfy), a `step` covering at least one and at most 32 bytes per pixel of the declared width, `data` of exactly `step × height` bytes, and those bytes actually present. A channel's declared schema is not proof of its bodies — a mislabelled topic, a truncated write or a stubbed payload would otherwise yield a fabricated resolution and a finding about honest data. Reported once per stream per episode. Silent where no image dimensions were read at all (every non-ROS format, and a `--metadata-only` run): a stream whose imagery was never measured is not a stream measured and found empty. |
| `autonomy.image-integrity` | `AUTONOMY.IMAGE_DROPPED` | warning | Some — not all — of a camera stream's frames declared no pixels: the camera cut out during the recording rather than never starting. Invisible to every timing check, because the empty frames keep the stream's rate and continuity intact, so the timeline says the camera was present over a stretch it did not see. Warning rather than error: the recording holds real imagery on either side of the dropout and may be usable once the affected span is cut. The finding names how many frames were empty, out of how many, and the largest size the frames that carried pixels declared. |
| `autonomy.image-integrity` | `AUTONOMY.IMAGE_RESIZED` | warning | A camera stream's frames do **not all declare the same size**. Nothing else notices — every frame is well-formed at both resolutions, the rate is unchanged and the timeline is continuous — so a resolution change part-way through a recording is a silent distribution shift. A policy with a fixed input shape trains on whichever size the loader resamples to, and the camera's intrinsics, calibrated at one resolution, are wrong for the other, so the same pixel projects to two different rays. The finding names the range the frames span. Reported once per stream per episode. |
| `autonomy.image-integrity` | `AUTONOMY.IMAGE_UNMEASURED` | info | A camera stream carries neither per-frame image dimensions nor container metadata, so the image rules had nothing to measure on it — the camera was never asked whether it recorded anything, which is indistinguishable in a report from being asked and coming back clean. A stream whose *container* was read (a video file's own header) is excluded: something did measure that imagery, and the `video.*` family reports on it. Reported once for the dataset, naming the streams. **Withheld under `--metadata-only`**: a run that did not open the message bodies read no dimensions for any stream of any format, so the finding would blame the data for a silence the request caused — `COVERAGE.METADATA_ONLY` states that instead. Informational, not a defect: a dataset is not worse for the container it was published in. What it changes is what a passing autonomy result is evidence of. |
| `autonomy.message-decode` | `AUTONOMY.MESSAGE_BODY_UNDECODED` | error | Some of a stream's **message bodies could not be decoded** — the bytes are present and the message's own invariants do not hold, which is what a truncated write, a dropped chunk, or a publisher whose message layout disagrees with its own declared sizes leaves behind. Every typed decoder in the ROS readers is strict on purpose: it yields a reading only once the body proves it is the message it claims to be, so a mislabelled topic or a stubbed payload cannot produce a fabricated number. This reports the other half of that guard. A body that fails it leaves behind a frame with a timestamp, a schema and a coordinate frame like any other — so the structural family sees frames, the temporal family sees a clean cadence, the frame checks place the sensor in the tree — and *everything read out of the bodies* (a stream's statistics, its point counts, its frame sizes, its capture stamps, its fix availability) is computed from whichever bodies survived and reported as a property of the whole stream. A rig recorded with four IMU messages in five and four GNSS messages in five cut short graded byte-identically to the healthy one. A **fault, not an abstention**: the bytes were reached and they are not the message, the same fact as `VIDEO.MEDIA_UNREADABLE` one format down. The neighbouring silence — a stream whose schema no typed decoder covers — carries no summary at all and is disclosed by the families that wanted to read it (`STATISTICAL.UNMEASURED_VALUES`, `AUTONOMY.POINT_CLOUD_UNMEASURED`, `AUTONOMY.IMAGE_UNMEASURED`), because "nothing failed" and "nothing was tried" are opposite facts. The finding names how many bodies failed out of how many were tried. Reported once per stream per episode. |
| `autonomy.sensor-clock` | `AUTONOMY.SENSOR_CLOCK_UNSET` | error / warning | A rig sensor's messages carry `header.stamp` of exactly zero — the epoch, which is what a driver that never set it publishes. Error when every message on the stream is unstamped, warning when the driver stopped stamping partway through. This matters because a bag has **two** clocks and only one of them reaches a frame timestamp: the recorder's, which times when a message *arrived*. The sensor's own capture time lives in `header.stamp`, and when it was never set there is nothing for the recorder's clock to be standing in for — so `AUTONOMY.RIG_SYNC`, `TEMPORAL.CLOCK_SKEW` and the start/end offsets are all measuring the recording host's scheduling, and a rig that is genuinely out of sync passes them. Nothing else reports it: the frames exist, the rate is clean and the stream grades perfectly. Scoped to streams that sample the world (the set `AUTONOMY.RIG_SYNC` compares), in rig episodes only. |
| `autonomy.sensor-clock` | `AUTONOMY.SENSOR_CLOCK_REGRESSION` | error | A rig sensor stamped a message earlier than the message recorded before it: the sensor's clock stepped backwards while the recorder's ran forwards — an NTP correction applied without slewing, or a device resetting its counter. Two different capture times then name the same instant, so anything fusing this sensor with another, or replaying the segment in capture order, silently reorders the observations across the step. Counted over the stream and reported once, naming how many messages regressed. |
| `autonomy.sensor-clock` | `AUTONOMY.SENSOR_CLOCK_OFFSET` | warning | A rig sensor's capture stamps sit further than the tolerance from the recorder's clock on **every** message (default 1 s, `sensor_clock_offset_ms`) — the two are not one clock, so this stream's capture times cannot be compared with any other sensor's on the rig. Judged from the smallest disagreement seen across the whole stream, not from an average, so a transient buffering spike cannot raise it and a constant pipeline latency of tens of milliseconds does not either. The default sits three orders of magnitude above real sensor latency and far below what an undisciplined clock produces (minutes to years), so it is nowhere near either edge. The frame timestamps hide the fault completely: they are all the recorder's. |
| `autonomy.sensor-clock` | `AUTONOMY.SENSOR_CLOCK_UNREAD` | info | A rig sensor stream records no capture time of its own, so the sensor-clock rules had nothing to compare the recording's timeline against — every non-ROS format, whose files carry one clock rather than one stamp per sample. Reported once for the dataset, naming the streams. **Withheld under `--metadata-only`**, which read no message bodies for any format; `COVERAGE.METADATA_ONLY` states that instead. Informational, not a defect: what it changes is what a passing sync result is evidence of — one clock agreeing with itself rather than two clocks agreeing with each other. |
...
| `statistical.value-measurability` | the adapter never read values (any MCAP or ROS bag topic — either generation — other than a `JointState`, an `Imu`, an `Odometry`'s velocity, or one of the messages that are nothing but their own reading — `Twist`/`Wrench` (and their stamped forms), `MagneticField`, `Temperature`, `FluidPressure`, `RelativeHumidity`, `Illuminance`, `Range`, and any `bytes_list` leaf of an RLDS record), or read them but had no stored statistics to compare against (HDF5, Zarr, CAN+DBC, MF4, RLDS numeric leaves, and a bag's measured topics) |
...
Fourteen further checks do their own work *and* disclose when they could not do it, rather than leaving a
...
| `autonomy.image-integrity` | `AUTONOMY.IMAGE_UNMEASURED` |
