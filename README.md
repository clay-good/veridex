...
the worst episode first, and the eight informational findings are elided here, `--full` adding their
risk and remedy too.
...
Veridex report
  ...
  By category: provenance 2 · semantic 2 · statistical 2 · structural 2 · temporal 2
...
- **One command, any format.** LeRobot (v2.0/2.1/3.0), RLDS/TFDS (what Open X-Embodiment ships in), HDF5 (what
  ...
  (candump text, Vector BLF — the binary format most vehicle traffic is recorded in — PCAN-Trace,
  any of them gzipped, classic frames and CAN-FD),
...
- **Catches the failures that quietly ruin training.** Clock skew across sensors, broken episode
  ...
  point clouds, the camera beside it that did the same and published a full recording of frames
  declaring no pixels, the camera that changed resolution part-way through so a fixed-input policy
  trains on whatever the loader resampled and the intrinsics fit neither half, the mobile base
  commanded at its speed limit for a whole run, the sensor whose message bodies did not survive the
  recording so every number reported about it was computed from the fifth that did, the vehicle-bus
  signal a mismatched database defines past the end of the frames the bus really carries so it
  decodes from none of them, the fleet measurement whose conditional signals the file itself marks
  invalid — each reported with the *training risk* it creates and a *remedy*.
...
# `av`, `av-miscalibrated`, `av-ambiguous-tf`, `av-dead-lidar`, `av-dead-camera`, `av-ego-jump`, `av-null-island`,
# `av-truncated-lidar`,
...
| **Sensor-rig checks** | Eleven checks over an autonomy rig, all cataloged in [docs/checks.md](docs/checks.md#autonomy--is-the-sensor-rig-internally-consistent) — this row is a summary, not the list. **Time:** rig-wide sync (`AUTONOMY.RIG_SYNC`) and per-sensor message loss — counted from the publisher's own per-message numbering where a source carries one, and estimated from the sensor's cadence where it does not. **Space:** the LiDAR-camera miscalibration a well-formed transform tree hides — a sensor whose own frame is absent from the tree or has no chain to the camera, an ego trajectory recorded for a body frame the tree never names, a tree that is *connected and still not a tree* because some frame has two parents at once, and — on a multi-camera rig — cameras sitting in two disconnected parts of that tree, where every sensor reaches the camera in its own part and nothing can be projected into the other. **Calibration that is present and cannot be used** is an error rather than a pass: the all-zero `CameraInfo` an uncalibrated driver publishes, a rotation quaternion that is not a unit quaternion (so it composes a scale into every placement), a principal point outside the image the calibration itself declares, distortion coefficients that do not fit their declared model. **Data:** a satellite fix outside the possible range or never acquired, a receiver that reported no fix for most of a drive — its own `STATUS_NO_FIX` is the only record of the outage, because those messages still arrive on time and carry no position, so the stream keeps the frame count, cadence and span of a healthy one — and a LiDAR whose driver lost its sensor and published well-formed empty point clouds at the right rate, which every other family passes on — and the same fault on the modality a policy is usually trained on: a camera publishing zero-sized frames, or one that changed resolution part-way through the recording, which shifts the training distribution without failing anything. **Bodies that did not survive the recording:** each of these decoders is strict on purpose — it reads a value only once the message's own invariants prove it is the message it claims to be — and the bodies that fail leave behind a frame with a timestamp, a schema and a frame, so a stream whose bodies were four fifths cut short by the recording is summarized from the fifth that survived and graded on it. **Two clocks, not one:** a bag times a message by when the recorder received it, while the sensor states when it *sampled* in the message's own header — so a sensor that never stamped its data, one whose clock steps backwards, or one that never disciplined to the recorder's clock leaves every sync result measuring the recording host rather than the rig. Each of these is judged only where the source states enough to judge it, and says so where it does not |
| **Video/media checks** | Read an `.mp4` or `.mkv`/`.webm` container's headers — never a pixel — and catch the missing, unparseable, desynced, or re-encoded video behind a camera stream. Also the one video fault an average cannot express: a camera that **stalled mid-recording**. Frames per second is frames over elapsed time, so a camera that froze for half a second in a twenty-second clip holds exactly as many frames as the episode has rows and still divides out to within two percent of its declared rate. `VIDEO.FRAME_GAP` reads the longest interval the container's own per-frame timing recorded — an MP4's `stts`, a Matroska's block timestamps — because a loader pairs frame *i* with row *i* by index and not by time, so the frames the camera never delivered are closed up rather than skipped and every pair after the gap is off |
...
| **Adapters** | LeRobot v2.0/2.1/3.0, RLDS/TFDS, HDF5, Zarr, MCAP, ROS 2 rosbag2, ROS 1 rosbag, CAN+DBC (candump, Vector BLF and PCAN-Trace; classic and FD), ASAM MDF/MF4 — with a passing cross-format neutrality gate (the same logical dataset yields equivalent CDMs as LeRobot v3 and as MCAP, and as rosbag2 and as MCAP) |
...
| **Check-packs** | A third party can register a named, versioned set of its own checks (`EngineBuilder::register_pack`) — a fleet's cadence rule, a site's frame budget — and **the verdict accounts for it**. Its checks are addressed under its own name (`mylab/house-rule`), the report and the JSON state which packs ran at what version, that record is part of the result content hash, and a certificate issued from such a run names them, because an offline reader cannot re-run Veridex to discover that "pass" was a pass under a catalog somebody extended. A run that loads no pack is unchanged in every respect, down to the result hash |
...
noise-perturbed copy shares no bytes and is out of reach without decoding. Where a stream carries no
frame fingerprint at all — a LeRobot video feature, whose pixels live in `.mp4` files outside the
Parquet — that stream is left out of the comparison and **named in the finding**, rather than
excluding the whole episode: requiring every stream to be hashed meant one video feature disabled
exact duplicate detection across an entire corpus, which is the ordinary shape of a real one. Every limit, and every
...
fingerprint, one whose video is not laid out per episode, one whose video states no per-frame timing
this reader can follow, one that offers no two streams on a shared
...
Seventeen checks in the catalog raise one; each declares which of its codes mean *"I could not measure"*
...
MCAP or to a bag of either ROS generation: a `sensor_msgs/msg/JointState`, a `sensor_msgs/msg/Imu`, the
`geometry_msgs/msg/Twist` a mobile base is driven by, the `Wrench` its gripper feels, and the
one-scalar readings beside them (`Temperature`, `Range`, …) carry nothing but
