...
- **One command, any format.** LeRobot (v2.0/2.1/3.0), RLDS/TFDS (what Open X-Embodiment ships in), HDF5 (what
  ...
  (candump text and Vector BLF, the binary format most vehicle traffic is recorded in, classic
  frames and CAN-FD),
...
| **Adapters** | LeRobot v2.0/2.1/3.0, RLDS/TFDS, HDF5, Zarr, MCAP, ROS 2 rosbag2, ROS 1 rosbag, CAN+DBC (candump and Vector BLF, classic and FD), ASAM MDF/MF4 — with a passing cross-format neutrality gate (the same logical dataset yields equivalent CDMs as LeRobot v3 and as MCAP, and as rosbag2 and as MCAP) |
