...
//!   `TEMPORAL.JITTER`.
//! - `frozen-episode` — five 10-frame episodes of a 3-DoF `action`; in episode 2 the arm never
//!   moved, so every one of its frames is byte-identical while the other four sweep. The
//!   teleoperation session that dropped: the episode is present, the right length, on time and
//!   evenly spaced, so no structural or temporal rule sees anything → `STRUCTURAL.FROZEN_EPISODE`.
...
pub fn describe(variant: &str) -> Result<&'static str, DemoError> {
    ...
    let what = match mode {
        ...
        Mode::FrozenEpisode => {
            "frozen-episode (nothing moved in episode 2 → STRUCTURAL.FROZEN_EPISODE)"
        }
        Mode::NearDuplicate => {
...
fn write_dataset(dir: &Path, mode: Mode) {
    ...
    let fps = 30.0;
    // The multi-DoF variants have their own info/parquet shape (a FixedSizeList feature).
    if mode == Mode::FrozenEpisode {
        write_frozen_episode_dataset(dir, fps);
        return;
    }
...
/// Write five 10-frame episodes of a 3-DoF `action` in which episode 2 never moved.
///
/// The teleoperation session that dropped, or the operator who never took the controls: the episode
/// is present, the right length, on time and evenly spaced, and its frames are byte-identical to one
/// another. Nothing structural or temporal sees it — only `STRUCTURAL.FROZEN_EPISODE`, which asks
/// whether an actuator *vector* stood still in a minority of the episodes carrying it.
///
/// A vector, not a scalar, because that is the question the check asks: one joint holding a position
/// is a joint at rest, and a whole arm that never moved is a recording of nothing. The other four
/// episodes sweep values of their own, so the frozen one is a minority and no two episodes are
/// duplicates.
fn write_frozen_episode_dataset(dir: &Path, fps: f64) {
    const EPISODES: usize = 5;
    const FRAMES: usize = 10;
    const FROZEN: usize = 2;
    let total = EPISODES * FRAMES;
    let info = serde_json::json!({
        "codebase_version": "v3.0",
        "fps": fps,
        "robot_type": "so100",
    ...
    let mut episodes: Vec<i64> = Vec::with_capacity(total);
    let mut values: Vec<f32> = Vec::with_capacity(total * 3);
    for ep in 0..EPISODES {
        for f in 0..FRAMES {
            episodes.push(ep as i64);
            if ep == FROZEN {
                // The arm held one pose for the whole recording. Inside the range the others sweep,
                // so it is not an outlier, and matching none of their values exactly, so no frame of
                // it is an accidental duplicate of theirs.
                values.extend_from_slice(&[7.5, -4.5, 0.25]);
            } else {
                let t = (ep * FRAMES + f) as f32;
                values.extend_from_slice(&[t * 0.5, -t * 0.3, t * 0.02]);
            }
        }
    }
    ...
    let batch = RecordBatch::try_new(
        ...
        vec![
            ...
            Arc::new(Int64Array::from((0..total as i64).collect::<Vec<_>>())),
            Arc::new(Float64Array::from(
                (0..total)
                    .map(|i| (i % FRAMES) as f64 / fps)
                    .collect::<Vec<_>>(),
            )),
            action,
        ],
