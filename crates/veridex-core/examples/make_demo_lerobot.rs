...
fn write_dataset(dir: &Path, mode: Mode) {
    ...
    // A Hugging Face-style dataset card so the adapter extracts a real license into provenance —
    // the demo then shows the `license` element covered instead of a MISSING_LICENSE gap.
    fs::write(
        dir.join("README.md"),
        "---\nlicense: apache-2.0\ntags:\n- robotics\n- lerobot\n---\n\n# Demo dataset\n\nSynthetic \
